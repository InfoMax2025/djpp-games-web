#!/usr/bin/env python3
"""Stamp shared files into each card game: djpp-kit.js, djpp-board.js, fixed sw.js (with a per-game cache name), _headers.
...
def build(name):
    ...
    board = (ROOT/'shared'/'djpp-board.js').read_text(encoding='utf-8')
    (d/'djpp-board.js').write_text(board, encoding='utf-8')
    assets = ['./', './index.html', './manifest.webmanifest', './djpp-kit.js', './djpp-board.js'] + g['icons']
